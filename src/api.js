...
function rewriteURLs($, destFilename) {
  ...
  $(".sidebar.primary a").each(function() {
    ...
    var depth = parseInt(
      $a.closest("li").attr("class").match(/\bdepth-([0-9]+)\b/)[1],
      10
    );
    ...
    if (isIndex) {
      ...
      href =
        "/api/" +
        destFilename.split("/")[0] +
        "/" +
        href.replace(/\.html$/, "").replace(/\./g, "_");
      $a.attr("href", href);
    }
  });
  ...
  $(".namespace a").each(function() {
    ...
    hrefParts[0] = hrefParts[0].replace(/\.html$/, "").replace(/\./g, "_");
...
module.exports = function(content, files, destFilename) {
  ...
  rewriteURLs(
    $,
    destFilename.replace(/^dest\/api\//, "").replace(/\/contents\.lr$/, "")
  );
